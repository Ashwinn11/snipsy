...
/// In-app legal documents. Postmark is fully offline, so both documents are
...
enum LegalDoc: String, Identifiable, CaseIterable {
    ...
    1. Agreement
    By downloading or using Postmark ("the app"), you agree to these Terms of Service. If you do not agree, please do not use the app.

    2. What Postmark Is
    Postmark turns photos you take into collectible stamp artworks that live in an album on your device. The app works entirely offline; it has no accounts, no server, and no social features.
    ...
    The short version
    Everything stays on your iPhone. Postmark has no accounts, no analytics, no ads, no tracking, and no server. We never see your photos — or anything else.

    1. Information We Collect
    None. The app does not collect, transmit, sell, or share any personal information. It makes no network requests.

    2. Camera
    Postmark asks for camera access so you can take photos inside the app. Frames from the camera are processed and displayed on your device only.
    ...
    6. Children
    Postmark does not collect data from anyone, including children.
